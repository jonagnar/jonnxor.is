...
- Content files are one file per locale: `<slug>.<locale>.md` (blog) / `<slug>.<locale>.yaml` (grimoire, games, pages, countdowns, wallpapers, projects). EVERY collection in `src/content.config.ts` MUST keep `generateId: localeEntryId` (from `src/content/loaders.ts`) or a slug's locale files collide on one id and silently overwrite each other — guarded by `tests/content/config-wiring.test.ts` (count updated per collection).
